...
interface IProps {
  TableMockData?: BatchOrderInterface;
  children?: string;
}
const DataStatus: React.FC<IProps> = ({ TableMockData, children }) => {
  return (
    <Box sx={{ display: "flex", marginRight: "100px" }}>
      <Box>
        <Link to={`/batchorderdetails/${TableMockData ? TableMockData.id : children}`}>
          <Button
            variant="contained"
            size="small"
            color="profileButton"
