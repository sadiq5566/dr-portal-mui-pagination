...
const BatchButtons: React.FC<IProps> = ({ user, dataId }) => {
  let disable =
    !user["reciept_number" + dataId] ||
    !user["confirm_number" + dataId] ||
    user["reciept_number" + dataId] !== user["confirm_number" + dataId] ||
    !user["total_amount" + dataId] ||
    !user["confirm_amount" + dataId] ||
    user["total_amount" + dataId] !== user["confirm_amount" + dataId]
      ? true
      : false;
    ...
    <Box sx={{ display: "flex" }}>
      <Box>
        <Button
          ...
          size="large"
          text="Complete"
          color={disable ? "batchDisable" : "success"}
          disabled={disable}
        />
      </Box>
      <Box>
        <Button variant="contained" size="large" color="batchReject" text="Reject" />
      </Box>
      <Buttons
        sx={{
          height: "30px",
